...
export function useGuardianLogic(callbacks: GuardianCallbacks = {}) {
  ...
  function debounceFaceDetected(rawFaceDetected: boolean): boolean {
    if (rawFaceDetected) {
      ...
      if (
        !debouncedFaceDetectedRef.current &&
  ...
  const processDetection = useCallback(
    (result: DetectionResult) => {
      ...
      if (state.gameState === 'CALIBRATING') {
        ...
        handlePlaying(debouncedFaceDetected, debouncedEyesOpen, isSmirking);
      }
   ...
   */
  function handlePlaying(faceDetected: boolean, eyesOpen: boolean, isSmirking: boolean) {
    // Check for face loss
    if (!faceDetected) {
