...
import { detectGPUCapability, selectFallbackStrategy, getFallbackConfig } from '../utils/gpuDetector';
...
export function useMediaPipe(options: UseMediaPipeOptions = {}): UseMediaPipeReturn {
  ...
  const initialize = useCallback(async () => {
    ...
    setState(prev => ({ ...prev, isLoading: true, error: null, loadingStage: 'initializing', loadingProgress: 0 }));
    ...
    try {
      ...
      resolutionManagerRef.current = new DynamicResolutionManager(
        640,
        480,
        {
          highQualityFPS: 45,
   ...
   */
  const handleDetectionResult = useCallback((payload: DetectionResult & { performance: PerformanceMetrics }) => {
   ...
   * Toggle GPU mode
   */
  const setGPU = useCallback((enabled: boolean) => {
    const worker = workerRef.current;
    if (!worker) return;
