...
function getUserFriendlyError(error, stage) {
  ...
  if (stage === 'gpu_initializing' || errorMessage.includes('WebGL') || errorMessage.includes('GPU')) {
...
async function handleInit(data) {
  ...
  try {
    console.log(`[MediaPipeWorker] Starting initialization (GPU: ${enableGPU}, retry: ${retryCount})`);
    ...
    try {
      ...
      console.log('[MediaPipeWorker] WASM loaded successfully');
    ...
    if (!cpuFallback && useGPU) {
      try {
        ...
        if (!navigator.gpu) {
          ...
          console.log('[MediaPipeWorker] GPU delegate available');
        }
      } catch (gpuError) {
    ...
    console.log(`[MediaPipeWorker] Creating FaceLandmarker with ${delegate} delegate...`);
    
    faceLandmarker = await FaceLandmarker.createFromOptions(
      cpuFallback ? await FilesetResolver.forVisionTasks(VISION_CDN) : await FilesetResolver.forVisionTasks(VISION_CDN),
      {
    ...
    console.log('[MediaPipeWorker] FaceLandmarker created successfully');
    ...
    console.log(`[MediaPipeWorker] Initialization complete. CPU fallback: ${cpuFallback}, Quality: ${currentQuality}`);
