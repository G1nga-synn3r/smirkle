...
class FaceData(BaseModel):
    ...
    bounding_box: Optional[Dict[str, float]] = Field(
        default=None, description="Face bounding box coordinates"
    )
    confidence: Optional[float] = Field(
        default=None, ge=0, le=1, description="Face detection confidence score"
    )
...
class ModelInfoResponse(BaseModel):
    ...
    available_models: List[Dict[str, str]] = Field(
        ..., description="Available recognition models"
    )
