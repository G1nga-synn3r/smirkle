...
class YouTubeIngestor:
    ...
    def _calculate_difficulty(self, view_count: int, like_count: int, duration_seconds: int) -> str:
        """
    ...
    def _map_to_smirkle_schema(self, video: dict) -> dict:
        ...
        preview_image = (
            ...
            f"{CONFIG['THUMBNAIL_BASE_URL'].format(video_id=video_id, quality='hqdefault')}"
        )
        
        # Estimate punchline timestamp (typically at 1/3 of video for comedic content)
        punchline_timestamp = round(duration_seconds * 0.33, 1) if duration_seconds > 3 else 0
    ...
    def _extract_tags(self, snippet: dict) -> List[str]:
        ...
        if not tags:
            ...
            # Common tag patterns
            keywords = ['funny', 'comedy', 'fail', ' Compilation', 'cats', 'dogs', 
                       'baby', 'animals', 'prank', 'humor', 'laugh', 'lol']
            tags = [kw for kw in keywords if kw in title][:5]
        
        return tags[:10]  # Ensure max 10 tags
    
    def _preflight_validation(self, video: dict) -> tuple[bool, str]:
        """
        ...
        if duration_seconds > CONFIG['MAX_DURATION_SECONDS']:
            return False, f"Duration exceeds {CONFIG['MAX_DURATION_SECONDS']}s limit ({duration_seconds}s)"
        
        # Check minimum duration (10s minimum for comedic content)
        if duration_seconds < CONFIG['MIN_DURATION_SECONDS']:
            return False, f"Duration too short ({duration_seconds}s), minimum {CONFIG['MIN_DURATION_SECONDS']}s required"
    ...
    def fetch_banger_videos(self, query: str = None, max_results: int = 25) -> List[dict]:
        """
        ...
        for search_query in queries:
            ...
            print(f"⏱️  Duration filter: {CONFIG['MIN_DURATION_SECONDS']}-{CONFIG['MAX_DURATION_SECONDS']}s (game loop optimized)")
            print("-" * 60)
        ...
        print(f"\n🏁 Total validated videos from all queries: {len(all_validated_videos)}")
    ...
    def save_to_staging(self, videos: List[dict], batch_id: str = None) -> str:
        ...
        staging_data['batch_info'] = {
            ...
            'pending_review': len([v for v in staging_data['videos'] if not v['safety_review']['approved']]),
            'approved': len([v for v in staging_data['videos'] if v['safety_review']['approved']]),
            'rejected': 0
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='YouTube Video Ingestor for Smirkle Project\n'
                    'Searches for high-intensity funny content optimized for the fast-paced game loop.',
...
Environment Variables:
  ...
  MAX_RESULTS      - Max results per query (default: 25)
    ...
    print(f"⏱️  Duration: {CONFIG['MIN_DURATION_SECONDS']}-{CONFIG['MAX_DURATION_SECONDS']}s (game loop optimized)")
    
    try:
        ingestor = YouTubeIngestor(CONFIG['YOUTUBE_API_KEY'])
        
        # Fetch videos (uses SEARCH_QUERIES if no custom query provided)
        videos = ingestor.fetch_banger_videos(query=args.query, max_results=args.max_results)
