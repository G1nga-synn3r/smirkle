...
class FacialExpressionDetector:
    ...
    def detect_expression(self, frame):
        ...
        try:
            ...
            happiness = emotions.get('happy', 0) / 100 + emotions.get('surprise', 0) / 100 * 0.3
    ...
    def process_frame(self, frame):
        ...
        if (current_time - self.last_detection_time) * 1000 >= self.config['detection_interval']:
            self.happiness_score, self.is_happy = self.detect_expression(frame[y:y+h, x:x+w])
        ...
        status += f" | Eyes: {'OPEN' : 'CLOSED'}"
        color = (0, 255, 0) if not self.is_happy else (0, 0, 255)
        cv2.putText(frame, status, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
        
        if self.is_happy:
            cv2.putText(frame, "🎬 SMIRK DETECTED - GAME OVER!", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        elif self.eyes_open:
            cv2.putText(frame, "▶️ VIDEO PLAYING - Keep a poker face!", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        else:
            cv2.putText(frame, "🔴 EYES CLOSED - Video paused", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 165, 255), 2)
    ...
    def log_detection(self):
        ...
        if self.is_happy:
            print(f"[{timestamp}] 😮 SMIRK DETECTED - Happiness: {self.happiness_score:.2%}, Eyes: {'OPEN' if self.eyes_open else 'CLOSED'}")
            ...
            while True:
                ...
                if game_active:
                    if self.check_game_over():
                        if not game_over:
                            print(f"\n😮 WASTED! You smiled at {self.happiness_score:.0%} happiness")
